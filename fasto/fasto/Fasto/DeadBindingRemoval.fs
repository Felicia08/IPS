...
let rec removeDeadBindingsInExp (e : TypedExp) : (bool * DBRtab * TypedExp) =
    match e with
        ...
        | Var (name, pos) ->
            ...
            *)
            let sym = recordUse name (SymTab.empty())
            (false, sym, Var (name, pos))
        | Plus (x, y, pos) ->
        ...
        | Index (name, e, t, pos) ->
            ...
            *)
            ...
            (io, (SymTab.combine sym uses), Index(name, e', t, pos))
        | Let (Dec (name, e, decpos), body, pos) ->
            ...
            *)
            ...
            if ((io2) = true) || isUsed name bodyuses then
                ((io1 || io2),SymTab.combine (SymTab.remove name bodyuses) euses, 
                Let (Dec (name, e', decpos), body', pos))
            else
                ((io1),bodyuses, body') // DET HER 
